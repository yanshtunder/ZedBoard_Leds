#include <linux/kernel.h>				/* Все-таки мы работаем с ядром! Определена функция printk */
#include <linux/module.h>				/* Используется всегда. Содержит функции и определения для динамической загрузки модулей ядра */
...
#include <linux/uaccess.h>				/* Предоставляет доступ к user space */
   

/* Как напишем имя, так оно и будет о тображаться в /proc */
 ...
 */
 ...
 static int proc_myled_open(struct inode *inode, struct file *file)
  ...
  /* Оперции, которые может выполнять драйвер */
  static const struct file_operations proc_myled_operations = {
      .open = proc_myled_open,						/* Открываем драйвер */
      .read = seq_read,								/* Читаются данные из драйвера. Функция предоставляется из <linux/seq_file.h> */
      .write = proc_myled_write,					/* Пишем в драйвер */
      .llseek = seq_lseek,
      .release = single_release						/* Закрываем драйвер */
  };
